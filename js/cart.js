...
window.addEventListener('click',function (event){
    ...
    if(event.target.hasAttribute('data-cart')){
        ...
        // Есть ли такой товар в корзинке
       //const ItemInCart = cartWrapper.querySelector(`[data-id="${productInfo.id}"]`);
       const ItemInCart = cartWrapper.querySelector('[data-id="' + productInfo.id + '"]');
        //console.log(ItemInCart);
        //если товар есть в корзинке
       if(ItemInCart){

        const counterElement = ItemInCart.querySelector('[data-counter]'); // элемент с счетчиком
         // складываем число из корзинки с тем числом из карты
       ...
       // пересчет обзей стоимости товаров в корзине
